 ...
 */
public class ARSurfaceView extends GLSurfaceView implements SensorEventListener, OnLocationUpdateListener, GeoARView,
     ...
     * @author Holger Hopmann
     */
    private class AROverlayDialog extends AlertDialog implements OnCheckedChangeListener {
	...
	public AROverlayDialog() {
	    ...
	    // Inflate Layout
	    View layout = LayoutInflater.from(getContext()).inflate(R.layout.ar_overlay_dialog, null);

	    // Find Button Views
	    ToggleButton buttonCalibration = (ToggleButton) layout.findViewById(R.id.toggleButtonOverlayCalib);
	    buttonCalibration.setChecked(renderer.showsCalibration());

	    buttonOverlayNoise = (ToggleButton) layout.findViewById(R.id.toggleButtonOverlayNoise);
	    ...
	    setButton(BUTTON_NEUTRAL, getContext().getString(R.string.ok), (Message) null);
	}
	...
	synchronized (rotMatrix) {
		...
		SensorManager.getRotationMatrix(rotMatrixSensor, null, accelValues.get(), magnetValues.get());
		// transforms from sensor to world

		switch (display.getOrientation()) {
		case Surface.ROTATION_0:
		    // No adjustment
		    SensorManager.remapCoordinateSystem(rotMatrixSensor, SensorManager.AXIS_X, SensorManager.AXIS_Y,
			    rotMatrix);
		    break;
		case Surface.ROTATION_90:
		    SensorManager.remapCoordinateSystem(rotMatrixSensor, SensorManager.AXIS_Y,
			    SensorManager.AXIS_MINUS_X, rotMatrix);
		    break;
		case Surface.ROTATION_180:
		    SensorManager.remapCoordinateSystem(rotMatrixSensor, SensorManager.AXIS_MINUS_X,
			    SensorManager.AXIS_MINUS_Y, rotMatrix);
		    break;
		case Surface.ROTATION_270:
		    SensorManager.remapCoordinateSystem(rotMatrixSensor, SensorManager.AXIS_MINUS_Y,
			    SensorManager.AXIS_X, rotMatrix);
		    break;
		}
	...
	if (!mSensorManager.registerListener(this, magnet, SensorManager.SENSOR_DELAY_GAME)) {
	    infoHandler.setStatus(R.string.magnetic_field_not_started, 5000, magnet);
	}
	if (!mSensorManager.registerListener(this, accel, SensorManager.SENSOR_DELAY_GAME)) {
	...
	if (accuracy != SensorManager.SENSOR_STATUS_ACCURACY_HIGH) {
		...
		infoHandler.setStatus(R.string.magnetic_field_not_calibrated, 5000, magnet);
	    } else if (sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
		infoHandler.setStatus(R.string.accel_not_calibrated, 5000, accel);
	    }
	}
    }
	...
	renderer.showCalibration(savedInstanceState.getBoolean("ARNoiseViewCalibration", false));
	renderer.showInterpolation(savedInstanceState.getBoolean("ARNoiseViewInterpolation", true));
    }
