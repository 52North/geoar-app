...
public class Geometry {
	...
	public void createBuffers() {
		if (verticesBuffer != null) {
			...
			createBuffer(verticesBuffer, BufferType.FLOAT_BUFFER,
					vertexDetails, GLES20.GL_ARRAY_BUFFER);
		}
		if (normalsBuffer != null) {
			normalsBuffer.compact().position(0);
			createBuffer(normalsBuffer, BufferType.FLOAT_BUFFER, normalDetails,
					GLES20.GL_ARRAY_BUFFER);
		}
		if (colorsBuffer != null) {
			colorsBuffer.compact().position(0);
			createBuffer(colorsBuffer, BufferType.FLOAT_BUFFER, colorDetails,
					GLES20.GL_ARRAY_BUFFER);
		}
		if (indicesBuffer != null) {
			indicesBuffer.compact().position(0);
			createBuffer(indicesBuffer, BufferType.INT_BUFFER, indicesDetails,
					GLES20.GL_ARRAY_BUFFER);
		}
	 ...
	 * 
	 * @param buffer
	 *            Buffer for interacting with NIO channels. A buffer is
	 *            essentially a block of memory into which data will be written
	 *            to in order to interacting with the graphical device.
	 * @param type
	 * @param details
	 * @param target
	 */
	public void createBuffer(Buffer buffer, BufferType type,
			BufferDetailsHolder details, int target) {
		int buff[] = new int[1];
		...
		GLES20.glBufferData(target, buffer.limit() * byteSize, buffer,
	...
	private static FloatBuffer allocateFloatMemory(float[] data, int byteSize) {
		FloatBuffer floatBuffer = ByteBuffer
				.allocateDirect(data.length * byteSize)
	...
	private static IntBuffer allocateIntMemory(int[] data, int byteSize) {
		IntBuffer intBuffer = ByteBuffer.allocateDirect(data.length * byteSize)
