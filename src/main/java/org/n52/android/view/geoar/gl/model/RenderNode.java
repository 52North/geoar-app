...
public abstract class RenderNode extends Spatial {
    ...
    public void onRender(float[] projectionMatrix, float[] viewMatrix, final float[] parentMatrix) {
	if (!isVisible)
	    return;
...
//	Matrix.translateM(modelMatrix, 0, 0f, 0f, 5f);
	...
	if (!isComposition) {
	    ...
	    GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, geometry.getIndicesDetails().bufferHandle);
	    // WTF! FU OPENGLES 2.0
//	    GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 0, geometry.getVerticesCount());
//	    GLES20.glDrawElements(drawingMode, geometry.getIndicesCount(), GLES20.GL_UNSIGNED_INT, 0);
    ...
    protected void setRenderObjectives(float[] vertices, float[] colors, float[] normals, int[] indices) {
	geometry.setRenderObjectives(vertices, colors, normals, indices);
    }
    
    protected void setRenderObjectives(float[] vertices, float[] colors, float[] normals, short[] indices) {
	geometry.setRenderObjectives(vertices, colors, normals, indices);
    }
