...
import org.n52.android.view.geoar.gl.ARSurfaceViewRenderer.GeoLocationUpdateListener;
import org.n52.android.view.geoar.gl.ARSurfaceViewRenderer.OnObservationUpdateListener;
...
public class PointsOfInteresst extends RenderNode implements OnObservationUpdateListener, GeoLocationUpdateListener {
	...
	public void run() {
		...
		public void run() {
			...
			float[] relativePosition = LocationConverter.getRelativePositionVec2(location,
				measure.getLongitude(), measure.getLatitude(), 0);
			float[] color = PointsOfInteresst.getColor(measure.getValue(), maxValue, minValue);
    ...
    private void updateRelativePosition(GeoPoint locationUpdate) {
	for (RenderNode rn : children) {
	    float[] newPosition = LocationConverter.getRelativePositionVec2(locationUpdate, rn.getLongitude(),
		    rn.getLatitude(), rn.getAltitude());
