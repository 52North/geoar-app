...
public class DataSourcesOverlay extends Overlay {
    ...
    private static final Map<Class<? extends OverlayType<?>>, Class<? extends DataSourceOverlay<? extends Geometry, ? extends OverlayType<?>>>> overlayClasses = new HashMap<Class<? extends OverlayType<?>>, Class<? extends DataSourceOverlay<? extends Geometry, ? extends OverlayType<?>>>>() {
        ...
        {
            put(PointOverlayType.class,
                    DataSourcePointOverlay.class);
            put(PolylineOverlayType.class,
                    DataSourcePolylineOverlay.class);
        }
    };
    ...
    // private Map<Object, List<DataSourceOverlay<? extends Geometry, ? extends
    // OverlayType<?>>>> overlayItemMap = Collections
    // .synchronizedMap(new HashMap<Object, List<DataSourceOverlay<? extends
    // Geometry, ? extends OverlayType<?>>>>());
    ...
    public void setOverlayItems(
        ...
        if (classMap == null) {
            classMap = createMap(key, (Class<? extends OverlayType<? extends Geometry>>) overlayItems  //
                            .get(0).getClass());                                            //
        }
        
        DataSourceOverlay<? extends Geometry, ? extends OverlayType<?>> dsOverlay = classMap.get(overlayItems.get(0).getClass());
        if (dsOverlay == null) {
            try {
                dsOverlay = overlayClasses.get(overlayItems.get(0).getClass()).newInstance();
                classMap.put((Class<? extends OverlayType<?>>) overlayItems.get(0).getClass(), dsOverlay);
    ...
    @SuppressWarnings("unchecked")
    protected Map<Class<? extends OverlayType<?>>, DataSourceOverlay<? extends Geometry, ? extends OverlayType<?>>> createMap(Object key,
            Class<? extends OverlayType<? extends Geometry>> class1) {
